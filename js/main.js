...
const cart = {
    ...
    addCartId(id) {
        ...
        if (goodItem) {
            ...
            const {id: index, name, price} = allGoods.find(good => good.id === id);
            this.cartGoods.push({id: index, name, price, count: 1});
            this.cartRender();
        }
    },
    cartRender() {
        ...
        this.cartGoods.forEach(good => {
            const {name, id, price, count} = good
    ...
    plusGood(id) {
        const element = this.cartGoods.find(el => el.id === id);
        if (element) {
            element.count++;
        }
        this.cartRender();
